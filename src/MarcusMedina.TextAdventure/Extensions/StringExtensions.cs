...
public static class StringExtensions
{
    ...
    {
        if (string.IsNullOrWhiteSpace(text) || string.IsNullOrWhiteSpace(other)) return maxDistance + 1;

        var left = text.Trim().ToLowerInvariant();
        var right = other.Trim().ToLowerInvariant();
        if (left.TextCompare(right)) return 0;
    ...
    {
        if (string.IsNullOrWhiteSpace(text)) return string.IsNullOrWhiteSpace(other) ? 0 : other!.Length;
        if (string.IsNullOrWhiteSpace(other)) return text!.Length;
        return LevenshteinDistanceCore(text.Trim(), other.Trim(), maxDistance);
    }
    ...
    {
        if (string.IsNullOrWhiteSpace(text) || string.IsNullOrWhiteSpace(other)) return false;
        return text.SoundexKey().TextCompare(other.SoundexKey());
    }
