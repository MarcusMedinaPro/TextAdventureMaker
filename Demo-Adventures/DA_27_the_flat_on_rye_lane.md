...
```adventure
world: The Flat on Rye Lane
goal: Reach the final turn with enough context to act decisively.
current_location: start

// --- DSL v2 entity definitions ---
define item: the_flat_on_rye_lane_note | Case Note | A folded note containing partial context and names.
define item: the_flat_on_rye_lane_decoy | Red Herring | A convincing but ultimately irrelevant object.
define item: the_flat_on_rye_lane_token | Proof Token | A concrete token that confirms what is really happening.
define npc: the_flat_on_rye_lane_watcher | Watcher | A tense observer who reacts to your choices.

// --- World layout ---
location: start | Arrive at the flat. Something feels wrong.
  ...
  item: the_flat_on_rye_lane_note | case note| A folded note containing partial context and names. | aliases=case,note
  exit: north -> crossroads
  exit: east -> sidetrack

location: crossroads | Explore the hallway. Notice the wallpaper is... breathing.
  npc: the_flat_on_rye_lane_watcher
  item: railing | iron railing| Cold iron slick with condensation. | takeable=false | aliases=iron,railing
  exit: south -> start
  exit: north -> finale

location: sidetrack | Enter the kitchen. The clock runs backwards.
  item: the_flat_on_rye_lane_decoy | red herring| A convincing but ultimately irrelevant object. | aliases=red,herring
  item: cabinet | utility cabinet| A paint-chipped cabinet full of mundane supplies. | takeable=false | aliases=utility,cabinet
  exit: west -> start

location: finale | Find a note in the bedroom. The handwriting is yours.
  item: the_flat_on_rye_lane_token | proof token| A concrete token that confirms what is really happening. | aliases=proof,token
  exit: south -> crossroads

// --- Start state ---
start_inventory: the_flat_on_rye_lane_note
start_stats: health=100 | resolve=50 | trust=0
flag: alarm_heard=false
counter: tension=0
relationship: the_flat_on_rye_lane_watcher=0
...
on_pickup: the_flat_on_rye_lane_token | effects="message:This detail is real. Build your choice around it."

npc_acceptance_default: the_flat_on_rye_lane_watcher | text="The watcher says little, but notes every move."
npc_rule: the_flat_on_rye_lane_watcher | when=counter.tension>=2 | priority=1 | then="message:The watcher glances away, as if expecting impact."
...
branch: sharp_end | when=has_item:the_flat_on_rye_lane_token | then="message:You reach the ending with evidence, not guesswork."
