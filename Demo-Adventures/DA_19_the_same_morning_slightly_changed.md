# Demo Adventure 19: The Same Morning, Slightly Changed
...
```adventure
world: The Same Morning, Slightly Changed
goal: Reach the final turn with enough context to act decisively.
current_location: start

// --- DSL v2 entity definitions ---
define item: the_same_morning_slightly_changed_note | Case Note | A folded note containing partial context and names.
define item: the_same_morning_slightly_changed_decoy | Red Herring | A convincing but ultimately irrelevant object.
define item: the_same_morning_slightly_changed_token | Proof Token | A concrete token that confirms what is really happening.
define npc: the_same_morning_slightly_changed_watcher | Watcher | A tense observer who reacts to your choices.
...
location: start | The disturbance arrives and feels personal.
  ...
  item: the_same_morning_slightly_changed_note | case note| A folded note containing partial context and names. | aliases=case,note
  exit: north -> crossroads
  exit: east -> sidetrack

location: crossroads | A rule is broken or a boundary is crossed.
  npc: the_same_morning_slightly_changed_watcher
...
location: sidetrack | A clue reveals the scale of the problem.
  item: the_same_morning_slightly_changed_decoy | red herring| A convincing but ultimately irrelevant object. | aliases=red,herring
...
location: finale | A choice narrows the world.
  item: the_same_morning_slightly_changed_token | proof token| A concrete token that confirms what is really happening. | aliases=proof,token
  exit: south -> crossroads

// --- Start state ---
start_inventory: the_same_morning_slightly_changed_note
start_stats: health=100 | resolve=50 | trust=0
flag: alarm_heard=false
counter: tension=0
relationship: the_same_morning_slightly_changed_watcher=0
...
on_pickup: the_same_morning_slightly_changed_token | effects="message:This detail is real. Build your choice around it."

npc_acceptance_default: the_same_morning_slightly_changed_watcher | text="The watcher says little, but notes every move."
npc_rule: the_same_morning_slightly_changed_watcher | when=counter.tension>=2 | priority=1 | then="message:The watcher glances away, as if expecting impact."
...
branch: sharp_end | when=has_item:the_same_morning_slightly_changed_token | then="message:You reach the ending with evidence, not guesswork."
...
SetupC64("The Same Morning, Slightly Changed");
