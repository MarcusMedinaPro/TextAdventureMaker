...
```adventure
world: Doorbell Shadow
goal: Reach the final turn with enough context to act decisively.
current_location: start

// --- DSL v2 entity definitions ---
define item: doorbell_shadow_note | Case Note | A folded note containing partial context and names.
define item: doorbell_shadow_decoy | Red Herring | A convincing but ultimately irrelevant object.
define item: doorbell_shadow_token | Proof Token | A concrete token that confirms what is really happening.
define npc: doorbell_shadow_watcher | Watcher | A tense observer who reacts to your choices.
...
location: start | You arrive and assess the immediate danger.
  ...
  item: doorbell_shadow_note | case note| A folded note containing partial context and names. | aliases=case,note
  exit: north -> crossroads
  exit: east -> sidetrack

location: crossroads | You inspect clues and identify a fragile route forward.
  npc: doorbell_shadow_watcher
...
location: sidetrack | A false lead wastes time but reveals useful context.
  item: doorbell_shadow_decoy | red herring| A convincing but ultimately irrelevant object. | aliases=red,herring
...
location: finale | You commit to a risky path with limited certainty.
  item: doorbell_shadow_token | proof token| A concrete token that confirms what is really happening. | aliases=proof,token
  exit: south -> crossroads

// --- Start state ---
start_inventory: doorbell_shadow_note
start_stats: health=100 | resolve=50 | trust=0
flag: alarm_heard=false
counter: tension=0
relationship: doorbell_shadow_watcher=0
...
on_pickup: doorbell_shadow_token | effects="message:This detail is real. Build your choice around it."

npc_acceptance_default: doorbell_shadow_watcher | text="The watcher says little, but notes every move."
npc_rule: doorbell_shadow_watcher | when=counter.tension>=2 | priority=1 | then="message:The watcher glances away, as if expecting impact."
...
branch: sharp_end | when=has_item:doorbell_shadow_token | then="message:You reach the ending with evidence, not guesswork."
