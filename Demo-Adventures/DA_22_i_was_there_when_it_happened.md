# Demo Adventure 22: I Was There When It Happened
...
```adventure
world: I Was There When It Happened
goal: Reach the final turn with enough context to act decisively.
current_location: start

// --- DSL v2 entity definitions ---
define item: i_was_there_when_it_happened_note | Case Note | A folded note containing partial context and names.
define item: i_was_there_when_it_happened_decoy | Red Herring | A convincing but ultimately irrelevant object.
define item: i_was_there_when_it_happened_token | Proof Token | A concrete token that confirms what is really happening.
define npc: i_was_there_when_it_happened_watcher | Watcher | A tense observer who reacts to your choices.
...
location: start | The disturbance arrives and feels personal.
  ...
  item: i_was_there_when_it_happened_note | case note| A folded note containing partial context and names. | aliases=case,note
  exit: north -> crossroads
  exit: east -> sidetrack

location: crossroads | A rule is broken or a boundary is crossed.
  npc: i_was_there_when_it_happened_watcher
...
location: sidetrack | A clue reveals the scale of the problem.
  item: i_was_there_when_it_happened_decoy | red herring| A convincing but ultimately irrelevant object. | aliases=red,herring
...
location: finale | A choice narrows the world.
  item: i_was_there_when_it_happened_token | proof token| A concrete token that confirms what is really happening. | aliases=proof,token
  exit: south -> crossroads

// --- Start state ---
start_inventory: i_was_there_when_it_happened_note
start_stats: health=100 | resolve=50 | trust=0
flag: alarm_heard=false
counter: tension=0
relationship: i_was_there_when_it_happened_watcher=0
...
on_pickup: i_was_there_when_it_happened_token | effects="message:This detail is real. Build your choice around it."

npc_acceptance_default: i_was_there_when_it_happened_watcher | text="The watcher says little, but notes every move."
npc_rule: i_was_there_when_it_happened_watcher | when=counter.tension>=2 | priority=1 | then="message:The watcher glances away, as if expecting impact."
...
branch: sharp_end | when=has_item:i_was_there_when_it_happened_token | then="message:You reach the ending with evidence, not guesswork."
...
SetupC64("I Was There When It Happened");
