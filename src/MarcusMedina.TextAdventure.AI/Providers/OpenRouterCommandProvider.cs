// <copyright file="OpenRouterCommandProvider.cs" company="Marcus Ackre Medina">
...
public sealed class OpenRouterCommandProvider : JsonHttpAiCommandProviderBase
{
    private readonly OpenRouterSettings _settings;

    public OpenRouterCommandProvider(OpenRouterSettings settings, HttpClient? client = null)
        : base(client, settings?.TimeoutMs ?? 8000)
    {
        _settings = settings ?? throw new ArgumentNullException(nameof(settings));
    }

    public override string Name => AiProviderNames.OpenRouter;
    ...
    public override Task<AiProviderResult> ParseAsync(AiParseRequest request, CancellationToken cancellationToken = default)
    {
        ...
        if (string.IsNullOrWhiteSpace(_settings.ApiKey))
            return Task.FromResult(AiProviderResult.Unavailable(Name, "OpenRouter API key is missing."));
